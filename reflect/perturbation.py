...
class ObjectReplacePerturbator:
    ...
    def perturb(self, task_suite_name: str, task_name: str, seed: Optional[int] = None) -> str:
        ...
        for obj_interest, candidates in task_cfg.items():
            ...
            # Filter out candidates that are the same as the original object
            valid_candidates = [c for c in candidates if c != obj_interest]
            
            if not valid_candidates:
                # If candidates list only contained the original object, skip
                # print(f"[物体替换] {obj_interest} 的候选列表只包含自身，跳过。")
                continue
        ...
        if not mapping:
            print("[物体替换] 没有形成任何有效替换映射（候选与原物体相同），跳过。")
            return self.parser.file_content
        ...
        for old_name, new_name in mapping.items():
            # Use regex with word boundaries to prevent substring replacement
            # e.g., prevent replacing "porcelain_mug" inside "white_porcelain_mug"
            pattern = r'\b' + re.escape(old_name) + r'\b'
            prefix = re.sub(pattern, new_name, prefix)
            suffix = re.sub(pattern, new_name, suffix)
...
def process_bddl_file_mixed(input_dir: str,
                            ...
                            suffix: str = "_temp") -> None: # 新增：输出目录的后缀
    """
        ...
        Args:
            ...
            flags (dict): 扰动参数标志。
            seed (int): 随机种子。
            suffix (str): 输出目录的后缀（默认为 "_temp"）。 #新增：输出目录的后缀
        """
    input_path = Path(input_dir)
    output_dir = input_path.parent / f"{input_path.name}{suffix}"
...
def create_env(
    ...
    :param suffix: 输出文件夹的后缀 # 新增：输出文件夹的后缀
    """
    # 默认扰动 flags
    flags = PerturbFlags(
